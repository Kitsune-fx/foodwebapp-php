<?php
require_once("config.php") ;

?>

<!DOCTYPE html>
<html>
    <head>
    <meta charset="utf-8">
        
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
        <title>ร้านหน้าปากซอย</title>
        <style>
            
            .logo {
                float:left;
                color:white;
                font-size:26px;
                cursor:pointer;
                text-decoration: none;
                
            }
                   .logo:hover {
                float:left;
                color:white;
                font-size:26px;
                cursor:pointer;
                text-decoration: none;
                
            }
            
            .header{
                width: 100%;
                top: 0;
                position: fixed;
                padding: 1em;
                margin-top: 0;
                text-align: right;
                background-color: black;
            }

            
            footer {
                 background-color:darkslateblue ;
                 padding: 1em;
                 width: 100%;
                 bottom: 0;
                 margin-bottom: 0 ;
                 position: fixed ;
                 text-align: center ;
            }
        </style>
    </head>
    
        <body>
            <div class="header">
                
              <a href="index.php" class="logo">
            FOODWEB
                </a>
           
             <span style="float:right;">
                 <a href="index.php" class="btn btn-warning">กลับสู่เมนูหลัก</a>
            </span>
            </div>
            
            
        <br><br><br><br><br><br><br>
           <legend> <span style="text-align:left;font-size:32px;">
                <div class="container">
                    เข้าสู่ระบบ  
                </div>
            </span></legend>
             <div class="container" style="font-size:25px;">
                 <form action="logincheck.php" method="post">  
                 
                 <br>
                   Username: <input type="text" class="form-control" style="width:50%;" name="mem_user" maxlength="20" required placeholder="sample" minlength="3"><br>
                   Password : <input type="password" class="form-control" style="width:50%;" name="mem_pwd" maxlength="20" required placeholder="8-20ตัวอักษร" minlength="8"><br>
                   <br>
                    <input type="submit" class="btn btn-success" value="เข้าสู่ระบบ">
                    <input type="reset" class="btn btn-default" value="รีเซต">
                   </form>
                 </div>
                <br><br><br>
            
    </body>
    
    
    
</html>
